...
const productJourneySteps = Array.from(document.querySelectorAll("[data-product-target]"));
const productColumns = Array.from(document.querySelectorAll("[data-product-category]"));
...
function setActiveProductCategory(category) {
  productJourneySteps.forEach((step) => {
    const isActive = step.dataset.productTarget === category;
    step.classList.toggle("is-linked", isActive);
    step.setAttribute("aria-pressed", String(isActive));
  });

  productColumns.forEach((column) => {
    column.classList.toggle("is-linked", column.dataset.productCategory === category);
  });
}
...
productJourneySteps.forEach((step) => {
  step.addEventListener("click", () => setActiveProductCategory(step.dataset.productTarget));
});

productColumns.forEach((column) => {
  column.addEventListener("click", () => setActiveProductCategory(column.dataset.productCategory));
  column.addEventListener("keydown", (event) => {
    if (event.key !== "Enter" && event.key !== " ") return;
    event.preventDefault();
    setActiveProductCategory(column.dataset.productCategory);
  });
});

renderStage("early");
setActiveProductCategory("ai");
